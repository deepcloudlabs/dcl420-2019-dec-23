...
class HRViewModel {
    constructor() {
        ...
        /*
        this.eventSource = new EventSource("http://localhost:8080/hr-backend/api/v1/employees/subscribe?user=james")
        this.eventSource.onopen = () => {
            toastr.success("SSE Connection is open!");
        };
        this.eventSource.onerror = (err) => {
            toastr.error(JSON.stringify(err));
        };
        this.eventSource.addEventListener('hr' , (event) =>{
           let newEmployee = JSON.parse(event.data);
           this.employees.push(newEmployee);
        });

         */

        this.websocket = new WebSocket("ws://localhost:8080/hr-backend/stream");
        this.websocket.onopen = (event) => {
            toastr.success("WebSocket Connection is open!");
        };
        this.websocket.onmessage = (event) => {
            let newEmp = JSON.parse(event.data);
